...
import supportnet.rule.domain.NoPairMailRule;
import supportnet.rule.domain.Schedule;

public class NoPairMailRuleAction  extends BaseRuleAction  {
	protected NoPairMailRule rule;
	
	public void prepare() throws JTException {
		super.prepare();
		if (rule != null && rule.getId() != null&&!"".equals(rule.getId())) {
			rule = (NoPairMailRule) service.findBoById(NoPairMailRule.class, rule.getId());
		}
	...
	public NoPairMailRule getRule() {
		return rule;
	}

	public void setRule(NoPairMailRule rule) {
		this.rule = rule;
	}
