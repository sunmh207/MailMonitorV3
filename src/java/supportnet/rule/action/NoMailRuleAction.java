...
import supportnet.rule.domain.NoMailRule;
import supportnet.rule.domain.Schedule;

public class NoMailRuleAction  extends BaseRuleAction  {
	protected NoMailRule rule;
	
	public void prepare() throws JTException {
		super.prepare();
		if (rule != null && rule.getId() != null&&!"".equals(rule.getId())) {
			rule = (NoMailRule) service.findBoById(NoMailRule.class, rule.getId());
		}
	...
	public NoMailRule getRule() {
		return rule;
	}

	public void setRule(NoMailRule rule) {
		this.rule = rule;
	}	
